...
public class CameraContainer extends RelativeLayout implements CameraOperation {
    ...
    public boolean startRecord() {
        ...
        if (mCameraView.startRecord()) {
            mHandler.postDelayed(recordRunnable, 1 * 1000);
            return true;
        } else {
            return false;
        }
        ...
        public void run() {
            if (mCameraView.isRecording()) {
                ...
                if (mProgressListener != null) {
                    mProgressListener.onProgressChanged(MAX_TIME, timeCount);
                }
                if (timeCount == MAX_TIME) {
                    if (mFinishListener != null) {
                        mFinishListener.onRecordFinish();
                    }
    ...
    public String getRecordPath() {
        return mCameraView.getRecordPath();
    }

    public Bitmap getVideoThumbnail(String path) {
        return mCameraView.getVideoThumbnail(path);
    }

    public OnRecordProgressListener mProgressListener;
