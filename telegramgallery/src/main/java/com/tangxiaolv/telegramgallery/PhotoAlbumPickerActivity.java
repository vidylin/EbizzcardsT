...
public class PhotoAlbumPickerActivity extends BaseFragment
    ...
    public interface PhotoAlbumPickerActivityDelegate {
        void didSelectPhotos(ArrayList<String> photos, ArrayList<String> captions, ArrayList<PhotoModel> photoModels);

        boolean didSelectVideo(MediaController.PhotoEntry photoEntry);
    ...
    private int selectedMode;// 0 图片 1 视频
...
//            layoutParams.leftMargin = AndroidUtilities.dp(dropDown.getTextSize());
            ...
            actionBar.setTitle(context.getString(R.string.PickerVideo));
            selectedMode = 1;
        } else {
    ...
    public void didReceivedNotification(int id, Object... args) {
        if (id == NotificationCenter.albumsDidLoaded) {
            ...
            if (!isVideo && null != albumsSorted && albumsSorted.size() > 0) {
                openPhotoPicker(albumsSorted.get(0), 0, true);
            } else if (isVideo && videoAlbumsSorted != null && videoAlbumsSorted.size() > 0) {
    ...
    private void sendSelectedPhotos() {
        ...
        for (MediaController.PhotoEntry photoEntry : selectedPhotosSortEnd) {
            ...
            photoModel.setOriginal(photoEntry.isOriginal);
            if (photoEntry.isOriginal) {
                photoModel.setOriginalPath(photoEntry.path);
            }else {
                photoModel.setOriginalPath(photoEntry.imagePath != null ? photoEntry.imagePath : photoEntry.path);
            }
            photoModel.setThumbnailPath(photoEntry.thumbPath);
            photoModel.setDescription(photoEntry.caption != null ? photoEntry.caption.toString() : null);
            photoModels.add(photoModel);
        }
        delegate.didSelectPhotos(photos, captions, photoModels);
    }
    ...
    private void openPhotoPicker(MediaController.AlbumEntry albumEntry, int type,
        ...
        currentPhotoPickerActivity
                .setDelegate(new PhotoPickerActivity.PhotoPickerActivityDelegate() {
                    ...
                    @Override
                    public boolean didSelectVideo(MediaController.PhotoEntry photoEntry) {
                        removeSelfFromStack();
                        return delegate.didSelectVideo(photoEntry);
                    }
