...
public class PhotoPickerActivity extends BaseFragment
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                if (selectedAlbum != null && selectedAlbum.isVideo) {
                    ...
                    if (delegate.didSelectVideo(selectedAlbum.photos.get(i))) {
                        finishFragment();
                    }
                } else {
    ...
    public boolean checkOriginalPhoto(boolean checked, int index) {
        int imageId = selectedAlbum.photos.get(index).imageId;
        if (!selectedPhotos.containsKey(imageId) && checked) {
            setPhotoChecked(index);
        }
        if (selectedPhotos.containsKey(imageId)) {
            MediaController.PhotoEntry photoEntry = selectedPhotos.get(imageId);
            photoEntry.isOriginal = checked;
    ...
    public boolean isCheckOriginal(int index) {
        int imageId = selectedAlbum.photos.get(index).imageId;
        if (selectedPhotos.containsKey(imageId)) {
            return selectedPhotos.get(imageId).isOriginal;
        }
        return false;
    }
