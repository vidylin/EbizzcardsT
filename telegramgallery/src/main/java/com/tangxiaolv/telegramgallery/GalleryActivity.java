...
public class GalleryActivity extends Activity implements ActionBarLayout.ActionBarLayoutDelegate {
    ...
    private PhotoAlbumPickerActivity.PhotoAlbumPickerActivityDelegate mPhotoAlbumPickerActivityDelegate = new PhotoAlbumPickerActivity.PhotoAlbumPickerActivityDelegate() {
        @Override
        public void didSelectPhotos(ArrayList<String> photos, ArrayList<String> captions, ArrayList<PhotoModel> photoModels) {
            Intent intent = new Intent();
            if (config.isSinglePhoto()) {
                intent.putExtra(PHOTO, photoModels.get(0).getOriginalPath());
            } else {
        ...
        public boolean didSelectVideo(MediaController.PhotoEntry photoEntry) {
            Intent intent = new Intent();
            intent.putExtra(VIDEO, photoEntry.path);
            setResult(Activity.RESULT_OK, intent);
            return true;
        }
