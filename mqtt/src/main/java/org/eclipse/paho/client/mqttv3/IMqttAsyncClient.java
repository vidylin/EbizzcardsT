...
public interface IMqttAsyncClient {
     ...
     * The token will be passed to the callback methods if a callback is set.
     ...
     * @see #connect(MqttConnectOptions, Object, IMqttActionListener)
     */
    IMqttToken connect(MqttConnectOptions options) throws MqttException;
     ...
     * </p>
     * <p>
     * The method returns control before the connect completes.
     * Completion can be tracked by:
     * <ul>
     ...
     */
    IMqttToken connect(MqttConnectOptions options, Object userContext, IMqttActionListener callback) throws MqttException;

    /**
     * Disconnects from the server.
     * <p>
     * An attempt is made to quiesce the client allowing outstanding work to complete before disconnecting.
     ...
     * </p>
     *
     * @param quiesceTimeout the amount of time in milliseconds to allow for existing work to finish before disconnecting.
     ...
     * </p>
     * <p>
     * The method returns control before the disconnect completes.
     * Completion can be tracked by:
     * <ul>
     * <li>Waiting on the returned token {@link IMqttToken#waitForCompletion()} or</li>
     * <li>Passing in a callback {@link IMqttActionListener}</li>
     ...
     * <p>
     * Because the client is able to establish the TCP/IP connection to a none MQTT server and
     * it will certainly fail to send the disconnect packet.
     * It will wait for a maximum of 30 seconds for work to quiesce before disconnecting and
     ...
     */
    IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained) throws MqttException;
     ...
     * @param retained    whether or not this message should be retained by the server.
     * @param userContext optional object used to pass context to the callback. Use null if not required.
     * @param callback    optional listener that will be notified when message delivery hsa completed to the requested quality of service
     ...
     * @see MqttMessage#setRetained(boolean)
     */
    IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained, Object userContext, IMqttActionListener callback) throws MqttException;

    /**
     * Publishes a message to a topic on the server.
     * Takes an {@link MqttMessage} message and delivers it to the server at the requested quality of service.
     *
     * @param topic   to deliver the message to, for example "finance/stock/ibm".
     ...
     * @see #publish(String, MqttMessage, Object, IMqttActionListener)
     */
    IMqttDeliveryToken publish(String topic, MqttMessage message) throws MqttException;
     ...
     * <li>Topic names can include the space character.  For example, <em>Accounts payable</em> is a valid topic.</li>
     * <li>
     * A leading "/" creates a distinct topic.
     * For example, <em>/finance</em> is different from <em>finance</em>. <em>/finance</em> matches "+/+" and "/+", but not "+".
     * </li>
     ...
     * </p>
     * <p>
     * The method returns control before the publish completes.
     * Completion can be tracked by:
     * <ul>
     * <li>
     * Setting an {@link IMqttAsyncClient#setCallback(MqttCallback)} where
     * the {@link MqttCallback#deliveryComplete(IMqttDeliveryToken)} method will be called.
     * </li>
     ...
     * @see MqttMessage
     */
    IMqttDeliveryToken publish(String topic, MqttMessage message, Object userContext, IMqttActionListener callback) throws MqttException;
     ...
     *                    Messages published at a higher quality of service will be received using the QoS specified on the subscribe.
     ...
     * @see #subscribe(String[], int[], Object, IMqttActionListener)
     */
    IMqttToken subscribe(String topicFilter, int qos, Object userContext, IMqttActionListener callback) throws MqttException;
     ...
     * </p>
     * <p>
     * The {@link #setCallback(MqttCallback)} method should be called before this method,
     * otherwise any received messages will be discarded.
     * </p>
     * <p>
     ...
     * <p>
     * <p>
     * The "topic filter" string used when subscribing may contain special characters,
     * which allow you to subscribe to multiple topics at once.
     * </p>
     * <p>
     ...
     * The multi-level wildcard and single-level wildcard can be used for subscriptions, but
     * they cannot be used within a topic by the publisher of a message.
     * <dl>
     ...
     * <p>
     * <p>
     * The <span>multi-level</span> wildcard can
     ...
     * wildcard matches only a single level, <em>finance/+</em> does not match <em>finance</em>.
     * </p>
     * <p>
     * <p>
     * Use the single-level wildcard at any level in the topic tree, and in conjunction
     ...
     * For example, <em>finance/+</em> and <em>finance/+/ibm</em> are both valid.</span>
     * </p>
     * </dd>
     * </dl>
     * </p>
     * <p>
     * The method returns control before the subscribe completes.
     * Completion can be tracked by:
     * <ul>
     ...
     */
    IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext, IMqttActionListener callback) throws MqttException;
     ...
     *
     * @param topicFilters one or more topics to unsubscribe from.
     *                     Each topicFilter must match one specified on an earlier subscribe.
     * @return token used to track and wait for the unsubscribe to complete.
     * The token will be passed to callback methods if set.
     * @throws MqttException if there was an error unregistering the subscription.
     * @see #unsubscribe(String[], Object, IMqttActionListener)
     ...
     * @throws MqttException if there was an error unregistering the subscription.
     */
    IMqttToken unsubscribe(String[] topicFilters, Object userContext, IMqttActionListener callback) throws MqttException;
     ...
     * After the client has been closed it cannot be reused.
     * For instance attempts to connect will fail.
     *
