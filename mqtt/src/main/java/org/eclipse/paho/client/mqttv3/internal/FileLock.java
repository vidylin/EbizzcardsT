 ...
 * If the lock is already held then an exception is thrown.
 * <p>
...
public class FileLock {
    ...
    public FileLock(File clientDir, String lockFilename) throws Exception {
        ...
        if (ExceptionHelper.isClassAvailable("java.nio.channels.FileLock")) {
            try {
                ...
                Method m = file.getClass().getMethod("getChannel");
                Object channel = m.invoke(file);
                m = channel.getClass().getMethod("tryLock");
                this.fileLock = m.invoke(channel);
            } catch (NoSuchMethodException | IllegalArgumentException | IllegalAccessException nsme) {
                this.fileLock = null;
            }
            if (fileLock == null) {
                // Lock not obtained
    ...
    public void release() {
        try {
            if (fileLock != null) {
                Method m = fileLock.getClass().getMethod("release");
                m.invoke(fileLock);
                fileLock = null;
            }
        } catch (Exception e) {
