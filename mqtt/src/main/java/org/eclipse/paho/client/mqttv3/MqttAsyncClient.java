...
public class MqttAsyncClient implements IMqttAsyncClient { // DestinationProvider {
    ...
    private static final long QUIESCE_TIMEOUT = 30_000; // ms
    private static final long DISCONNECT_TIMEOUT = 10_000; // ms
     ...
     * constructor.
     *
     * @param serverURI the address of the server to connect to, specified as a URI. Can be overridden using {@link MqttConnectOptions#setServerURIs(String[])}
     ...
     * @param clientId    a client identifier that is unique on the server being connected to
     * @param persistence the persistence class to use to store in-flight message. If null then the default persistence mechanism is used
     * @throws IllegalArgumentException if the URI does not start with "tcp://", "ssl://" or "local://"
     * @throws IllegalArgumentException if the clientId is null or is greater than 65535 characters in length
     * @throws MqttException            if any other problem was encountered
     */
...
//                }
    ...
    public IMqttToken disconnect(long quiesceTimeout, Object userContext, IMqttActionListener callback) throws MqttException {
        ...
        log.fine(CLASS_NAME, methodName, "104", new Object[]{quiesceTimeout, userContext, callback});
     ...
     * <li>
     * A leading "/" creates a distinct topic.  For example, <em>/finance</em> is
    ...
    protected MqttTopic getTopic(String topic) {
        ...
        MqttTopic result = topics.get(topic);
        if (result == null) {
    ...
    public void reconnect() throws MqttException {
        ...
        log.fine(CLASS_NAME, methodName, "500", new Object[]{clientId});
    ...
    private void startReconnectCycle() {
        ...
        log.fine(CLASS_NAME, methodName, "503", new Object[]{clientId, Long.valueOf(reconnectDelay)});
    ...
    private void stopReconnectCycle() {
        ...
        log.fine(CLASS_NAME, methodName, "504", new Object[]{clientId});
    ...
    private void rescheduleReconnectCycle(int delay) {
        ...
        log.fine(CLASS_NAME, methodName, "505", new Object[]{clientId, reconnectDelay});
        reconnectTimer.schedule(new ReconnectTask(), reconnectDelay);
    }
    ...
    public void setBufferOpts(DisconnectedBufferOptions bufferOpts) {
        comms.setDisconnectedMessageBuffer(new DisconnectedMessageBuffer(bufferOpts));
    }

    public int getBufferedMessageCount() {
        return comms.getBufferedMessageCount();
    }

    public MqttMessage getBufferedMessage(int bufferIndex) {
        return comms.getBufferedMessage(bufferIndex);
    }

    public void deleteBufferedMessage(int bufferIndex) {
        comms.deleteBufferedMessage(bufferIndex);
    }
