...
public class CommsCallback implements Runnable {
    ...
    private final Vector<MqttPublish> messageQueue;
    ...
    private void handleActionComplete(MqttToken token) throws MqttException {
        ...
        synchronized (token) {
            ...
            if (!token.internalTok.isNotified()) {
                // If a callback is registered and delivery has finished call delivery complete callback.
            ...
            if (token.isComplete()) {
                if (token instanceof MqttDeliveryToken || token.getActionCallback() != null) {
     ...
     * only added to the queue for inbound messages if the client is not quiescing.
     *
     ...
     * added to the process queue and let existing work quiesce. (until the thread is told to shutdown).
     */
    public void quiesce() {
    ...
    public boolean isQuiesced() {
        return quiescing && completeQueue.size() == 0 && messageQueue.size() == 0;
    }
    ...
    public void messageArrivedComplete(int messageId, int qos) throws MqttException {
        if (qos == 1) {
            this.clientComms.internalSend(new MqttPubAck(messageId), new MqttToken(clientComms.getClient().getClientId()));
        } else if (qos == 2) {
