...
public class ClientComms {
     ...
     * Does not check if connected this validation must be done by invoking routines.
     *
    ...
    /**
     * Sends a message to the broker if in connected state, but
     * only waits for the message to be stored, before returning.
     */
    ...
    public void shutdownConnection(MqttToken token, MqttException reason) {
        ...
        MqttToken endToken; //Token to notify after disconnect completes
        ...
        // con or discon which may be returned and will be notified at the end
        ...
        // All disconnect logic has been completed allowing the client to be marked as disconnected.
        synchronized (conLock) {
        ...
        synchronized (conLock) {
            if (closePending) {
                try {
                    close();
                } catch (Exception ignored) { // ignore any errors as closing.
                }
            }
        }
    }
    ...
    // disconnect or connect token to notify after disconnect is complete.
        ...
        try {
            ...
            while (toksToNotE.hasMoreElements()) {
                ...
                if (tok.internalTok.getKey().equals(MqttDisconnect.KEY) ||
                    ...
                    // that a problem has occurred and disconnect is in progress
    ...
    // Kick off the disconnect processing in the background so that it does not block.
    // For instance the quiesce.
     ...
     * so that the callbacks are attached as soon as the token is created (Bug 473928)
     */
