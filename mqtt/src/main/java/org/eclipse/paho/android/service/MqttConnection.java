...
class MqttConnection implements MqttCallbackExtended {
     ...
     * @param persistence  the persistence class to use to store in-flight message. If null
    ...
    public void connect(MqttConnectOptions options, String invocationContext, String activityToken) {
        ...
        try {
            ...
            if (mqttAsyncClient != null) {
                if (isConnecting) {
                    service.traceDebug(TAG, "mqttAsyncClient != null and the client is connecting. Connect return directly.");
                    service.traceDebug(TAG, "Connect return:isConnecting:" + isConnecting + ".disconnected:" + disconnected);
                } else if (!disconnected) {
                    service.traceDebug(TAG, "mqttAsyncClient != null and the client is connected and notify!");
                    doAfterConnectSuccess(resultBundle);
                } else {
                    service.traceDebug(TAG, "mqttAsyncClient != null and the client is not connected");
                    service.traceDebug(TAG, "Do Real connect!");
                    setConnectingState(true);
                    mqttAsyncClient.connect(connectOptions, invocationContext, listener);
                }
            }

            // if mqttAsyncClient is null, then create a new connection
            else {
                alarmPingSender = new AlarmPingSender(service);
                mqttAsyncClient = new MqttAsyncClient(serverURI, clientId, persistence, alarmPingSender);
    ...
    private void deliverBacklog() {
        Iterator<StoredMessage> backlog = service.messageStore.getAllArrivedMessages(clientHandle);
     ...
     *
     * @param messageId the message's identifier in the messageStore, so that
     *                  a callback can be made to remove it once delivered
    ...
    void disconnect(long quiesceTimeout, String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                mqttAsyncClient.disconnect(quiesceTimeout, invocationContext, listener);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else {
    ...
    void disconnect(String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                mqttAsyncClient.disconnect(invocationContext, listener);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else {
    ...
    public IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained, String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            ...
            try {
                ...
                sendToken = mqttAsyncClient.publish(topic, payload, qos, retained, invocationContext, listener);
    ...
    public IMqttDeliveryToken publish(String topic, MqttMessage message, String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                sendToken = mqttAsyncClient.publish(topic, message, invocationContext, listener);
                storeSendDetails(topic, message, sendToken, invocationContext, activityToken);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else if ((mqttAsyncClient != null) && (this.bufferOpts != null) && (this.bufferOpts.isBufferEnabled())) {
    ...
    public void subscribe(final String topic, final int qos, String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                mqttAsyncClient.subscribe(topic, qos, invocationContext, listener);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else {
     ...
     */
    public void subscribe(final String[] topic, final int[] qos, String invocationContext, String activityToken) {
        service.traceDebug(TAG, "subscribe({" + Arrays.toString(topic) + "}," + Arrays.toString(qos) + ",{" + invocationContext + "}, {" + activityToken + "}");
        final Bundle resultBundle = new Bundle();
        resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.SUBSCRIBE_ACTION);
        resultBundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken);
        resultBundle.putString(MqttServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext);
    ...
    public void subscribe(String[] topicFilters, int[] qos, String invocationContext, String activityToken, IMqttMessageListener[] messageListeners) {
        service.traceDebug(TAG, "subscribe({" + Arrays.toString(topicFilters) + "}," + Arrays.toString(qos) + ",{" + invocationContext + "}, {" + activityToken + "}");
        final Bundle resultBundle = new Bundle();
        resultBundle.putString(MqttServiceConstants.CALLBACK_ACTION, MqttServiceConstants.SUBSCRIBE_ACTION);
        resultBundle.putString(MqttServiceConstants.CALLBACK_ACTIVITY_TOKEN, activityToken);
        resultBundle.putString(MqttServiceConstants.CALLBACK_INVOCATION_CONTEXT, invocationContext);
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                mqttAsyncClient.subscribe(topicFilters, qos, messageListeners);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else {
    ...
    void unsubscribe(final String topic, String invocationContext, String activityToken) {
        ...
        if ((mqttAsyncClient != null) && (mqttAsyncClient.isConnected())) {
            IMqttActionListener listener = new MqttConnectionListener(resultBundle);
            try {
                mqttAsyncClient.unsubscribe(topic, invocationContext, listener);
            } catch (Exception e) {
                handleException(resultBundle, e);
            }
        } else {
     ...
     * @param messageToken      IMqttDeliveryToken
     * @param invocationContext String
     * @param activityToken     String
     */
    ...
    synchronized void reconnect() {
        ...
        if (mqttAsyncClient == null) {
            service.traceError(TAG, "Reconnect mqttAsyncClient = null. Will not do reconnect");
            return;
        }
        ...
        if (connectOptions.isAutomaticReconnect()) {
            ...
            try {
                ...
                mqttAsyncClient.connect(connectOptions, null, listener);
                setConnectingState(true);
            } catch (MqttException e) {
                ...
                /*  TODO: Added Due to: https://github.com/eclipse/paho.mqtt.android/issues/101
                    For some reason in a small number of cases, mqttAsyncClient is null here and so
                    a NullPointer Exception is thrown. This is a workaround to pass the exception
                    up to the application. mqttAsyncClient should not be null so more investigation is
                    required.
                */
                service.traceError(TAG, "Cannot reconnect to remote server." + e.getMessage());
                setConnectingState(false);
     ...
     *
     * @param bufferOpts DisconnectedBufferOptions
     */
    ...
    MqttMessage getBufferedMessage(int bufferIndex) {
        return mqttAsyncClient.getBufferedMessage(bufferIndex);
    }
