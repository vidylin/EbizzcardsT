...
public class SmartRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent, NestedScrollingChild {
    ...
    protected boolean mEnableAutoRefresh = false;//是否在列表滚动到顶部时自动刷新
    ...
    protected boolean mRefreshFinished = false;//数据是否全部加载完成，如果完成就不能在触发加载事件
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mRefreshHeader != null  && mEnableRefresh && !mEnablePureScrollMode) {
            boolean isInEditMode = isInEditMode() && mEnablePreviewInEditMode;
            final View headerView = mRefreshHeader.getView();
            final LayoutParams lp = (LayoutParams) headerView.getLayoutParams();
        ...
        if (mRefreshFooter != null && mEnableLoadmore && !mEnablePureScrollMode) {
            boolean isInEditMode = isInEditMode() && mEnablePreviewInEditMode;
            final View footerView = mRefreshFooter.getView();
            final LayoutParams lp = (LayoutParams) footerView.getLayoutParams();
                ...
                if (mVerticalPermit) {
                    ...
                    if (finay > 0) {// 手势向上划 Footer
                        if (mEnableLoadmore || mEnableOverScrollDrag) {
                            if (!mEnablePureScrollMode && mEnableAutoLoadmore && mEnableLoadmore && !mLoadmoreFinished) {
//                                autoLoadmore(0, 1);
...
//                mLastTouchY = touchY;
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mEnableAutoRefresh && mEnableRefresh
                            && spinner > 0
                            && mState != RefreshState.Refreshing
                            && mState != RefreshState.Loading
                            && mState != RefreshState.RefreshFinish
    ...
    protected void setStateDirectRefreshing() {
        if (mState != RefreshState.Refreshing) {
...
//        }
    ...
    public void onNestedScroll(final View target, final int dxConsumed, final int dyConsumed, final int dxUnconsumed, final int dyUnconsumed) {
        ...
        if (mState == RefreshState.Refreshing || mState == RefreshState.Loading) {
            ...
            if (mEnableRefresh && dy < 0 && (mRefreshContent == null || mRefreshContent.canRefresh())) {
                if (mState == RefreshState.None && !mRefreshFinished) {
                    setStatePullDownToRefresh();
                }
                mTotalUnconsumed += Math.abs(dy);
                moveSpinnerInfinitely(mTotalUnconsumed);
    ...
    /**
     * 设置是否监听列表在滚动到顶部时触发刷新事件
     */
    @Override
    public RefreshLayout setEnableAutoRefresh(boolean enable) {
        this.mEnableAutoRefresh = enable;
        return this;
    }
    ...
    /**
     * 设置数据全部刷新完成，将不能再次触发刷新功能
     */
    @Override
    public SmartRefreshLayout setRefreshFinished(boolean finished) {
        mRefreshFinished = finished;
        if (mRefreshHeader != null) {
            mRefreshHeader.setRefreshFinished(finished);
        }
        return this;
    }
     ...
     */
    @Override
    public SmartRefreshLayout resetLoadNoMoreData() {
     ...
     */
    @Override
    public RefreshLayout resetRefreshNoMoreData() {
    ...
    public boolean autoRefresh(int delayed, final float dragrate) {
        if (mState == RefreshState.None && (mEnableRefresh && !mRefreshFinished)) {
            if (reboundAnimator != null) {
                reboundAnimator.cancel();
            }
