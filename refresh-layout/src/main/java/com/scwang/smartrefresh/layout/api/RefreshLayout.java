...
public interface RefreshLayout {
    ...
    /**
     * 设置是否监听列表在滚动到顶部时触发刷新事件（默认false）
     */
    RefreshLayout setEnableAutoRefresh(boolean enable);

    /**
     * 设置数据全部刷新完成，将不能再次触发刷新功能（true）
     * @deprecated 请使用 {@link #finishRefreshWithNoMoreData()} 和 {@link #resetRefreshNoMoreData()} 代替
     */
    RefreshLayout setRefreshFinished(boolean finished);
     ...
     * @deprecated 请使用 {@link #finishLoadmoreWithNoMoreData()} 和 {@link #resetLoadNoMoreData()} 代替
     */
    @Deprecated
     ...
     * 恢复没有更多刷新的原始状态
     */
    RefreshLayout resetRefreshNoMoreData();
     ...
     * 恢复没有更多数据的原始状态
     */
    RefreshLayout resetLoadNoMoreData();
