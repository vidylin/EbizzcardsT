    ...
    private void updateSurfaceSize() {
        ...
        synchronized (layoutLock) {
            if (enableFixedSize && rotatedFrameWidth != 0 && rotatedFrameHeight != 0 && getWidth() != 0 && getHeight() != 0) {
                ...
                logD("updateSurfaceSize. Layout size: " + getWidth() + "x" + getHeight()
...
//                    getHolder().setFixedSize(width, height);
                    getHolder().setFixedSize(Math.min(getWidth(), 720), Math.min(getHeight(), 1280));// FIXME 临时解决: 使用固定宽高修复帧渲染抖动问题
                }
            } else {
    ...
    public void updateFrameDimensionsAndReportEvents(VideoRenderer.I420Frame frame) {
        synchronized (layoutLock) {
