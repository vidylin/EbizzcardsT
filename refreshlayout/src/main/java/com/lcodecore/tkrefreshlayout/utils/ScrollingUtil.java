...
public class ScrollingUtil {
    ...
    public static boolean isViewToTop(View view, int mTouchSlop) {
        ...
        if (view instanceof ScrollView || view instanceof WebView) return isScrollViewOrWebViewToTop(view);
        return (view != null && Math.abs(view.getScrollY()) <= 1.5 * mTouchSlop);
    }
    ...
    public static boolean isWebViewToBottom(WebView webview, int mTouchSlop) {
        return webview != null && ((webview.getContentHeight() * webview.getScale() - (webview.getHeight() + webview.getScrollY())) <= 1.5 * mTouchSlop);
    }
        ...
        if (recyclerView != null) {
            ...
            if (manager instanceof LinearLayoutManager) {
                ...
                if (layoutManager.findLastCompletelyVisibleItemPosition() == layoutManager.getItemCount() - 1) {
                    ...
                    View child = findOneVisibleChild(recyclerView.getChildCount() - 1, -1, true, false, layoutManager, recyclerView);
                    if (child != null && layoutManager.getPosition(child) == layoutManager.getItemCount() - 1) {
                        return true;
                    }
                }
            } else if (manager instanceof StaggeredGridLayoutManager) {
    ...
    public static View findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible,
                             boolean acceptPartiallyVisible, LinearLayoutManager layoutManager, RecyclerView recyclerView) {
        OrientationHelper orientationHelper = OrientationHelper.createOrientationHelper(layoutManager, layoutManager.getOrientation());
