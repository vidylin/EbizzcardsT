...
//    private int mExHeadHeight = 0;
    ...
    private static String sDEFAULT_HEADER_CLASS_NAME = "";
    private static String sDEFAULT_FOOTER_CLASS_NAME = "";
    ...
    public TwinklingRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            mMaxHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_head_height, DensityUtil.dp2px(context, 220));
            mMaxBottomHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_max_bottom_height, DensityUtil.dp2px(context, 220));
            mHeadHeight = a.getDimensionPixelSize(R.styleable.TwinklingRefreshLayout_tr_head_height, DensityUtil.dp2px(context, 60));
    ...
    private void addHeader() {
        ...
        addView(extraHeadLayout, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));

        FrameLayout headViewLayout = new FrameLayout(getContext());
        LayoutParams headLP = new LayoutParams(LayoutParams.MATCH_PARENT, 0);
        headLP.addRule(ALIGN_PARENT_TOP);
        addView(headViewLayout, headLP);

        mExtraHeadLayout = extraHeadLayout;
        mHeadLayout = headViewLayout;

        if (mHeadView == null) {
            if (!TextUtils.isEmpty(sDEFAULT_HEADER_CLASS_NAME)) {
                try {
                    Class<?> headClazz = Class.forName(sDEFAULT_HEADER_CLASS_NAME);
    ...
    private void addFooter() {
        ...
        LayoutParams bottomLP = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0);
        bottomLP.addRule(ALIGN_PARENT_BOTTOM);
        bottomViewLayout.setLayoutParams(bottomLP);
        addView(bottomViewLayout);

        mBottomLayout = bottomViewLayout;

        if (mBottomView == null) {
            if (!TextUtils.isEmpty(sDEFAULT_FOOTER_CLASS_NAME)) {
                try {
                    Class<?> clazz = Class.forName(sDEFAULT_FOOTER_CLASS_NAME);
    ...
    private int mMaximumFlingVelocity = -1;
    private int mMinimumFlingVelocity = -1;
    ...
    public static void setDefaultHeader(String className) {
        sDEFAULT_HEADER_CLASS_NAME = className;
    }

    //设置默认的footer class 名
    public static void setDefaultFooter(String className) {
        sDEFAULT_FOOTER_CLASS_NAME = className;
    }
    ...
    public boolean isRefreshing() {
        return cp.isRefreshing();
    }

    public boolean isLoading() {
        return cp.isLoadingMore();
    }
